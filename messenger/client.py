...
class Client(metaclass=ClientVerifier):
    ...
    def __init__(self):
        ...
        try:
            ...
            client_logger.warning('Значения адреса и порта по умолчанию')
        except ValueError:
            client_logger.critical('Порт в пределах от 1024 до 65535')
            sys.exit(1)

    @Log()
    # функция presence-сообщение
    ...
    def check_response(self, message, CONFIGS):
        if CONFIGS.get('RESPONSE') in message:
            ...
            client_logger.error('ошибка ответа сервера')
    ...
    def create_user_message_to_all(self, sock, CONFIGS, username='Guest'):
        ...
        client_logger.debug(f'Cловарь сообщения: {message_dict}')
        try:
